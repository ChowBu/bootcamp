...
https://www.cnblogs.com/linxiyue/p/7456004.html
"""
...
class QiniuStorage(Storage):
    ...
    # 必需的。
    # 被Storage.open()调用，这是储存类用于打开文件的实际工具。
    # 它必须返回File对象，在大多数情况下，你会想要返回一些子类，
    # 它们实现了后端储存系统特定的逻辑。
    def _open(self, name, mode='rb'):
        return QiniuFile(name, self, mode)

    # 必需的。
    # 被Storage.save()调用。name必须事先通过get_valid_name()
    # 和 get_available_name()过滤，并且content自己必须是一个File对象。
    # 应该返回被保存文件的真实名称（通常是传进来的name，但是如果储存需要修改文件名称，
    # 则返回新的名称来代替）。
    ...
    def _put_file(self, name, content):
        ...
        ret, info = put_file(token, name, content)
