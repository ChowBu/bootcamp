...
# token = q.upload_token(bucket_name, request.user.username + '_tmp.jpg', 3600)
# ret, info = put_file(token, request.user.username + '_tmp.jpg', filename)
# print(info)

def upload_put_file( upload_name, local_file):
    auth = Auth(access_key, secret_key)
    token = auth.upload_token(bucket_name,upload_name)
    ret, info = put_file(token, upload_name, local_file)
    if ret is None or ret['key'] != upload_name:
        raise info
...
def upload_picture(request):
    try:
        ...
        if width > 350:
            ...
            im.save(filename)
        upload_put_file(request.user.username + '_tmp.jpg',filename)
        upload_put_file(request.user.username + '.jpg',filename)
